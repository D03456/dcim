...
<!-- issue-fixes-240-241-242 -->

## 付録A. Issue対応追補: ER/共通カラム方針

### A.1 ポリモーフィック参照

`resource_alias` や `tagged_resource` の `resource_type + resource_id` は通常FKではなく、Service層検証を前提とする汎用参照である。ER図上では関連を点線で表現し、物理FKは張らない。

### A.2 システム処理時の監査カラム

`created_by` / `updated_by` はNOT NULLを原則とし、操作ユーザーが存在しない処理ではシステムユーザーIDを設定する。

| 処理 | 設定値 |
|---|---|
| Flyway初期データ | 固定のシステムユーザーID |
| バッチ処理 | システムユーザーID |
| 通知自動生成 | システムユーザーID |
| システム管理者操作 | 実操作者のユーザーID |

システムユーザーIDはアプリログ・監査ログ上で通常ユーザーと区別できる表示名を持たせる。
