...
| タグ管理 | SCR-025 | 共通タグ管理 |
| 通知設定・管理者向け通知履歴 | SCR-026 | 通知条件・通知先、保守期限/プラン上限/招待通知履歴管理 |
...
## 付録A. Issue対応追補: メニュー入口と初期遷移

ログイン後メニュー、認証前リンク、将来拡張/初期追加の非表示項目を分離する。

| 分類 | 対象 |
|---|---|
| 認証前リンク | パスワード再設定、招待承諾 |
| ログイン後メニュー | ダッシュボード、データセンター、ラック、機器、IP、保守契約、タグ、通知、ユーザー、契約プラン、操作履歴、システム設定、リージョン、連絡先、横断検索 |
| システム管理者メニュー | テナント一覧、テナント登録・編集、システム運用通知 |
| 初期追加で表示 | CSVインポート |
| 将来拡張で表示 | クラウド管理 |

遷移図で孤立している管理系画面は、左サイドメニューまたは管理メニューからの入口を持つものとして扱う。システム管理者はログイン後、通常ダッシュボードではなくテナント一覧または専用ダッシュボードへ遷移する。

<!-- issue-fixes-282-285 -->

## 付録B. Issue対応追補: 限定検索・契約変更申請遷移

- 契約管理者が横断検索を利用する場合、結果から通常資産詳細画面へは遷移させない。詳細リンクは非表示とし、URL直接アクセスも権限で拒否する。
- プラン/オプション変更申請の承認・却下を画面で行う場合は、システム管理者メニューから申請一覧/詳細へ遷移する導線を追加する。
- 承認・却下を外部運用/APIで扱う場合、画面遷移図には申請作成までを描き、承認処理は画面対象外として注記する。
