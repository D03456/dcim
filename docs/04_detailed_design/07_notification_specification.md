...
60日前、30日前、当日は、同一契約・同一通知タイミング・同一チャネル・同一宛先につき1回のみ送信する。期限切れ通知は初回送信後7日ごとに再通知し、`timing_code=EXPIRED` と `reference_date`（再通知周期開始日）を重複キーに含める。停止条件は保守契約の更新状態が `RENEWED` または `TERMINATED`、通知無効化、契約削除、終了日の更新で期限切れでなくなった場合とする。
...
契約管理者は保守期限通知の初期対象には含めず、プラン上限警告/到達通知およびFreeトライアル期限通知の対象とする。DC連絡先・機器担当者は初期通知対象には含めない。必要になった場合は将来拡張で追加する。
...
<!-- issue-fixes-224-225-230 -->

## 付録A. Issue対応追補: 通知ログ・操作エラー通知

### A.1 パスワード再設定・招待通知のログ保存禁止事項

`PASSWORD_RESET`、`USER_INVITATION` では、通知ログにトークン付きURL、トークン値、本文全文を保存しない。`action_url` はNULLまたはトークンを除去した画面種別のみとし、`body` はマスキング済み要約に限定する。
...
| トークン付きURL | 保存禁止 |
| メール本文全文 | 原則保存禁止 |
| 宛先メール | 必要に応じてマスキングまたはハッシュ化 |
| 通知種別・送信結果 | 保存する |

### A.2 OPERATION_ERROR通知先

| 発生元 | メール通知先 | 画面内通知先 | 備考 |
|---|---|---|---|
| CSV_EXPORT_FAILED | 実行ユーザー、テナント管理者 | 実行ユーザー、テナント管理者 | 権限があるユーザーのみ |
| CSV_IMPORT_FAILED | 実行ユーザー、テナント管理者 | 実行ユーザー、テナント管理者 | 初期追加フェーズ |
| MAIL_SEND_FAILED | テナント管理者、運用管理者、必要に応じてシステム管理者 | テナント管理者、運用管理者 | 外部連絡先には送らない |
| BATCH_FAILED | テナント管理者、運用管理者、システム管理者 | テナント管理者、運用管理者、システム管理者 | テナント特定不可の場合はシステム管理者のみ |
| DB_BACKUP_FAILED / SYSTEM_ERROR | システム管理者 | システム管理者 | テナント通知は行わない |

同一発生元・対象・当日内の連続失敗は集約し、`occurrence_count` と `retry_required` を更新する。

### A.3 通知ログ保持期間

初期リリースの暫定方針は以下とする。正式な運用設計で変更する場合も、実装前に保持期間を確定する。

| 対象 | 暫定保持期間 | 整理方針 |
|---|---:|---|
| 通知ログ成功 | 180日 | 期限超過後にアーカイブまたは削除 |
