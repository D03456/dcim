...
<!-- issue-fixes-292-293-294-296 -->

## 付録C. Issue対応追補: 識別子重複・パスワード・ユーザー上限

### C.1 CIDR範囲重複

IpSubnetのCIDRはcanonical化後に範囲へ変換し、同一テナント・同一IPバージョン内で既存有効サブネットと範囲重複しないことを検証する。`10.0.0.0/24` と `10.0.0.128/25` のような包含関係も不可とする。

### C.2 識別子ごとの重複判定範囲

| 項目 | 判定範囲 |
|---|---|
| ユーザーメール | システム全体で削除済み/無効化済みを含めて重複禁止。ただし将来の再利用方針は別途決定 |
| 機器シリアル番号 | 同一テナント内で削除済みを含めて重複禁止 |
| 保守契約番号 | 同一テナント内で削除済みを含めて重複禁止。NULLは複数許容 |
| IPサブネットCIDR | 同一テナント内で削除済みを含めて範囲重複禁止 |
| 有効IPアドレス | 同一テナント内で削除済みを含めて重複禁止 |
| 名称・表示名・タグ名 | 有効データ内で重複禁止。削除後の再利用は許容 |

### C.3 パスワードValidation

新パスワードは最小12文字、最大128文字、英大文字/英小文字/数字/記号のうち3種類以上を満たすこと。メールアドレス、氏名、テナント名を含む文字列、代表的な弱い文字列、直近パスワードの再利用は禁止する。

### C.4 ユーザー数上限

ユーザー招待時は、`ACTIVE` ユーザー数 + 有効期限内の `INVITED` 招待数を上限カウントに含める。期限切れ・取消済み招待、無効化済みユーザーは含めない。再招待は既存招待の更新として扱う。
