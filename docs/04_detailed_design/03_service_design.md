...
<!-- issue-fixes-292-294-296 -->

## 付録C. Issue対応追補: CIDR・パスワード・ユーザー上限

### C.1 CIDR範囲重複チェック

IpSubnetServiceは、CIDR登録/更新時にcanonical化したCIDRから `ipVersion`, `rangeStart`, `rangeEnd` を算出し、同一テナント内の有効サブネットと範囲重複しないことを検証する。完全一致だけでなく、包含関係・部分重複も不可とする。

### C.2 パスワードポリシー

AuthApplicationService / UserServiceは、招待承諾・パスワード再設定・管理者による初期設定時に同じパスワードポリシーを適用する。
...
| ユーザー情報 | メールアドレス、氏名、テナント名を含むものは禁止 |
| 過去パスワード | 初期リリースでは直近1件の再利用禁止。履歴保持を拡張する場合は別途設計 |
| 漏えい済みチェック | 初期はローカル禁止リスト/代表的な弱い文字列を拒否。外部漏えいDB連携は将来拡張 |

### C.3 ユーザー数上限

PlanLimitServiceのユーザー数上限判定には、`ACTIVE` ユーザーと有効期限内の `INVITED` 招待を含める。期限切れ招待、取消済み招待、無効化済みユーザーはカウントしない。再招待は既存招待の更新として扱い、新規ユーザー枠を二重消費しない。
