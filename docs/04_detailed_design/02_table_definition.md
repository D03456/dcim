...
## 付録B. Issue対応追補: 認証境界・契約番号制約

### B.1 メールログインとテナント境界

初期リリースでは、ログイン画面はメールアドレスとパスワードのみを入力するため、通常ユーザーのメールアドレスはシステム全体で有効データ一意とする。システム管理者も同じ `app_user` で管理し、`tenant_id = NULL` を許容する場合でも `email` は全体一意とする。

| 項目 | 方針 |
|---|---|
| 通常ログイン | `email` で有効ユーザーを一意に特定する |
| テナント識別子入力 | 初期リリースでは不要 |
| 複数テナント同一メール | 初期リリースでは禁止 |
| システム管理者 | `tenant_id = NULL` でも同一email重複は禁止 |

物理制約はMariaDBの論理削除方針に合わせ、有効ユーザーのみ `email` 一意になる生成列等で表現する。

### B.2 保守契約番号の重複禁止

`maintenance_contract.contract_number` は、値がある場合、同一テナント内で削除済みを含めて重複禁止とする。NULLは「契約番号未設定」として複数許容する。

| 制約 | 方針 |
|---|---|
| NULL | 複数許容 |
| NOT NULL | `tenant_id + contract_number` で重複禁止 |
| 論理削除後 | 契約番号の再利用不可 |
